...
export default function BranchFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        if (refetchBranches) refetchBranches(); // обновим контекст
        router.push('/admin/branches');
      } else {
      ...
      <form onSubmit={handleSubmit} className="bg-surface-card rounded-2xl shadow-card border border-border p-6 space-y-5">
        ...
        <div className="flex justify-end gap-3 pt-4">
          <button
            ...
            className="px-5 py-2 rounded-lg border border-border text-text-secondary hover:bg-surface-hover"
          >
            {t('cancel')}
          </button>
          <button
            type="submit"
            disabled={saving}
            className="px-5 py-2 rounded-lg bg-primary text-white font-medium hover:opacity-90 disabled:opacity-50"
          >
